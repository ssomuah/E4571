...
if [ ! "$NOTEBOOK_DIR" = "" ]; then
  ...
  if [[ "$NOTEBOOK_DIR" == s3://* ]]; then
    ...
    if [ "$NOTEBOOK_DIR_S3_S3NB" = true ]; then
      ...
      sudo python3 -m pip install s3contents
      cd /mnt
      ...
      echo "c.S3ContentsManager.access_key_id = 'AKIAJU267MQGRTAKQWTA'" >> ~/.jupyter/jupyter_notebook_config.py
      echo "c.S3ContentsManager.secret_access_key = '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'" >> ~/.jupyter/jupyter_notebook_config.py
...
sudo cp ~/.jupyter/jupyter_notebook_config.py /etc/jupyter/jupyter_notebook_config.py
fi

 s3-dist-cp --src="s3://colon/E4751/" --dest="hdfs:///user/jupyter/E4751" --s3Endpoint=s3.us-west-2.amazonaws.com
